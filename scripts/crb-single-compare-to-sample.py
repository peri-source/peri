...
pl.figure()
pl.loglog(rads, crbs[:,0], 'o-', lw=1.5, c=colors[0], alpha=0.6, label='CRB pos-z')
pl.loglog(rads, crbs[:,1], 'o-', lw=1.5, c=colors[1], alpha=0.6, label='CRB pos-y')
pl.loglog(rads, crbs[:,2], 'o-', lw=1.5, c=colors[2], alpha=0.6, label='CRB pos-x')
pl.loglog(rads, crbs[:,3], 'o-', lw=1.5, c=colors[3], alpha=0.6, label='CRB rad')

pl.errorbar(rads, stdmu[:,0], yerr=stderr[:,0], fmt='o', lw=1.5, c=colors[0], label='STD pos-z')
pl.errorbar(rads, stdmu[:,1], yerr=stderr[:,1], fmt='o', lw=1.5, c=colors[1], label='STD pos-y')
pl.errorbar(rads, stdmu[:,2], yerr=stderr[:,2], fmt='o', lw=1.5, c=colors[2], label='STD pos-x')
pl.errorbar(rads, stdmu[:,3], yerr=stderr[:,3], fmt='o', lw=1.5, c=colors[3], label='STD rad')
